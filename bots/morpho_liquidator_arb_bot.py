"""Bot de liquidações Morpho Blue na chain Arbitrum.
...
Contrato Morpho Blue Arbitrum (chain 42161):
...
_fh = logging.FileHandler(os.path.join(_LOG_DIR, "morpho_arb.log"))
...
# Stablecoins Arbitrum (minúsculas): (symbol, decimals, usd_price)
_STABLE_TOKENS: dict[str, tuple[str, int, float]] = {
    "0xaf88d065e77c8cc2239327c5edb3a432268e5831": ("USDC",   6, 1.0),  # USDC native
    "0xfd086bc7cd5c481dcc9c85ebe478a1c0b69fcbb9": ("USDT",   6, 1.0),  # USDT
    "0xda10009cbd5d07dd0cecc66161fc93d7c9000da1": ("DAI",   18, 1.0),  # DAI
    "0xff970a61a04b1ca14834a43f5de4533ebddb5cc8": ("USDC.e", 6, 1.0),  # USDC bridged
}
...
class MorphoLiquidatorArbBot:
    """Monitoriza e (em modo live) executa liquidações Morpho Blue no Arbitrum."""

    def __init__(self, settings: dict | None = None):
        self.settings = settings or get_settings()
        self.cfg = self.settings.get("bots", {}).get("morpho_liquidator_arb", {})

        primary_rpc = get_env("ALCHEMY_ARB_URL") or _FALLBACK_RPC_1
        ...
        self._ws_thread     = threading.Thread(
            target=self._ws_runner, daemon=True, name="morpho-arb-ws")
        ...
        logger.info(
            "MorphoArb: rpc=%s dry_run=%s hf<%.2f min_profit=$%.2f",
    ...
    def _switch_rpc(self, failed_url: str) -> bool:
        for url in self._rpc_urls:
            ...
            try:
                ...
                logger.warning("MorphoArb: RPC → %s", url.split("//")[-1].split("/")[0])
                return True
            except Exception:
                continue
        return False
        ...
        for attempt in range(3):
            try:
                ...
                if status == 429 and attempt == 0:
                    logger.warning("MorphoArb: rate-limit (429) — fallback RPC")
                    if self._switch_rpc(self._active_rpc):
                        continue
                time.sleep(2 ** attempt)
            except Exception as exc:
                logger.debug("MorphoArb: erro RPC: %s", exc)
    ...
    def _oracle_price_raw(self, oracle_address: str) -> int | None:
        ...
        try:
            ...
            logger.debug("MorphoArb: oracle %s…: %s", oracle_address[:10], exc)
            return None
    ...
    def _scan_borrowers(self) -> None:
        try:
            ...
            logger.warning("MorphoArb: bloco actual: %s", exc)
            return
        ...
        while from_block < latest:
            ...
            try:
                ...
                logger.debug("MorphoArb: Borrow [%d..%d]: %s", from_block, to_block, exc)
                break
        ...
        logger.info(
            "MorphoArb: scan %d..%d +%d posições (total=%d, %d mercados)",
    ...
    def _check_position(self, market_id: str,
                        borrower: str) -> LiqOpportunityMorphoArb | None:
        ...
        try:
            ...
            # 7. LIF e lucro estimado (gas em ETH)
            ...
            return LiqOpportunityMorphoArb(
            ...
            logger.debug("MorphoArb: %s:%s…: %s", market_id[:8], borrower[:10], exc)
            return None

    # ── registo na BD ─────────────────────────────────────────────────────────

    def _record(self, opp: LiqOpportunityMorphoArb) -> None:
        ...
        logger.debug(
            "MorphoArb: BD %s id=%d %s/%s HF=%.4f liq=%s profit=$%.2f",
    ...
    def _execute_live(self, opp: LiqOpportunityMorphoArb, nonce: int) -> str | None:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "MorphoArb: simulação falhou %s — cooldown 2min: %s",
            ...
            logger.info("MorphoArb: LIQUIDATE TX: %s", tx_hash.hex())
            return tx_hash.hex()
        except Exception as exc:
            self._cooldown[_cd_key] = time.time() + 300
            logger.warning("MorphoArb: liquidate revertida — cooldown 5min: %s", exc)
            return None
        ...
        while not self._ws_stop.is_set():
            ...
            try:
                async with websockets.connect(
                    ...
                    logger.info("MorphoArb: WS newHeads subscrito @ %s (id=%s)",
                ...
                logger.warning("MorphoArb: WS erro @ %s — reconecta em 5s: %s",
    ...
    def tick(self) -> list[dict]:
        ...
        if not self._positions or not _do_check:
            return []
        ...
        try:
            ...
            logger.warning("MorphoArb: get_nonce falhou — tick saltado: %s", exc)
            return []
        if not self.dry_run:
            try:
                _bal_wei = self.w3.eth.get_balance(_acct.address)
            except Exception as exc:
                logger.warning("MorphoArb: get_balance falhou — tick saltado: %s", exc)
                return []
            if _bal_wei < Web3.to_wei(0.005, 'ether'):
                logger.error(
                    "MorphoArb: saldo insuficiente (%.6f ETH < 0.005) — execução suspensa",
                    float(Web3.from_wei(_bal_wei, 'ether')),
                )
                return []

        _now_tick = time.time()
        logger.info("MorphoArb Tick: bloco=%d %d posições | %d blacklist",
        ...
        for (market_id, borrower) in list(self._positions.keys()):
            ...
            if _cd_key in self._blacklist:
                ...
                if abs(opp.health_factor - _bl_hf) / max(_bl_hf, 0.001) < 0.05:
                    logger.debug("MorphoArb: %s blacklisted — saltado",
                                 borrower[:10] + "…")
                    continue
                del self._blacklist[_cd_key]
                self._fail_counts.pop(_cd_key, None)
                logger.info("MorphoArb: %s saiu da blacklist", borrower[:10] + "…")

            tx_hash = None
            is_liq = opp.health_factor < 1.0
            if is_liq:
                logger.info(
                    "MorphoArb: LIQUIDÁVEL %s %s/%s HF=%.4f dívida=$%.2f lucro≈$%.4f dry=%s",
                ...
                if not self.dry_run and opp.estimated_profit_usd >= self.min_profit:
                    ...
                    if _until > _now_tick:
                        logger.debug("MorphoArb: %s em cooldown (%.0fs)",
                        ...
                        if tx_hash is None:
                            ...
                            if _cnt >= _BLACKLIST_FAILS:
                                ...
                                logger.warning(
                                    "MorphoArb: %s adicionado à blacklist (%d falhas)",
                                    borrower[:10] + "…", _cnt,
                                )
                        else:
                            self._fail_counts.pop(_cd_key, None)
        ...
        logger.info(
            "MorphoArb: tick — %d liquidáveis / %d em vigilância "
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("MORPHO BLUE ARBITRUM LIQUIDATOR — Smoke Test")
    print("=" * 60)

    bot = MorphoLiquidatorArbBot()
