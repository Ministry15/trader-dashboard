"""Bot de liquidações Aave V3 na chain Arbitrum One.

Estratégia idêntica ao aave_liquidator_avax_bot.py, mas independente:
  ...
  3. Posições com HF < health_factor_threshold (default: 1.2):
       ...
       – Estima custo de gas (Arbitrum usa ETH)
  ...
  5. DRY_RUN=false: chama contrato de flash loan para executar
       (requer flash_loan_contract em settings.yaml aave_liquidator_arb)

Contratos Aave V3 Arbitrum One (mainnet, chain 42161):
...
_BONUS: dict[str, float] = {
    "0x82af49447d8a07e3bd95bd0d56f35241523fbab1": 0.050,  # WETH    → 5%
    "0x2f2a2543b76a4166549f7aab2e75bef0aefc5b0f": 0.075,  # WBTC    → 7.5%
    "0xaf88d065e77c8cc2239327c5edb3a432268e5831": 0.050,  # USDC    → 5%
    "0xff970a61a04b1ca14834a43f5de4533ebddb5cc8": 0.050,  # USDC.e  → 5%
    "0xfd086bc7cd5c481dcc9c85ebe478a1c0b69fcbb9": 0.050,  # USDT    → 5%
    "0xda10009cbd5d07dd0cecc66161fc93d7c9000da1": 0.050,  # DAI     → 5%
    "0x5979d7b546e38e414f7e9822514be443a4800529": 0.075,  # wstETH  → 7.5%
    "0x912ce59144191c1204e64559fe8253a0e49e6548": 0.050,  # ARB     → 5%
    "0x35751007a407ca6feffe80b3cb397736d2cf4dbe": 0.075,  # weETH   → 7.5%
}
...
class AaveLiquidatorArbBot:
    """Monitoriza e (em modo live) executa liquidações Aave V3 na Arbitrum One."""

    def __init__(self, settings: dict | None = None):
        self.settings = settings or get_settings()
        self.cfg = self.settings.get("bots", {}).get("aave_liquidator_arb", {})
        ...
        logger.info(
            "AaveArb: rpc=%s dry_run=%s hf<%.2f min_profit=$%.2f",
    ...
    def _switch_rpc(self, failed_url: str) -> bool:
        ...
        for url in self._rpc_urls:
            ...
            try:
                ...
                logger.warning("AaveArb: RPC trocado para fallback: %s",
                               url.split("//")[-1].split("/")[0])
                return True
            except Exception:
                continue
        ...
        for attempt in range(3):
            try:
                ...
                if status == 429:
                    if attempt == 0:
                        logger.warning(
                            "AaveArb: rate-limit (429) — a tentar fallback RPC (tentativa %d/3)",
                            attempt + 1,
                        )
                        if self._switch_rpc(self._active_rpc):
                            continue
                    wait = 2 ** attempt
                    logger.debug("AaveArb: 429 rate-limit, aguardar %ds…", wait)
                    time.sleep(wait)
                else:
                    logger.debug("AaveArb: HTTP %s ao verificar ligação: %s", status, exc)
                    return False
            except Exception as exc:
                logger.debug("AaveArb: erro ao verificar ligação RPC: %s", exc)
    ...
    def _eth_price(self) -> float:
        ...
        try:
            raw = self.oracle.functions.getAssetPrice(
                Web3.to_checksum_address(WETH_ARB)).call()
            self._eth_price_cache = raw / 10 ** _ORACLE_DECIMALS
            self._eth_price_ts    = now
        except Exception as exc:
            logger.debug("AaveArb: oracle ETH price falhou: %s — cache $%.2f",
    ...
    def _gas_price_gwei(self) -> float:
        try:
            ...
            return 0.1   # Arbitrum: ~0.1 Gwei típico
    ...
    def _calc_gas_price(self, net_profit_usd: float) -> int:
        ...
        try:
            ...
            base_fee = int(0.1 * 1e9)  # 0.1 gwei fallback (Arbitrum)
        ...
        logger.info(
            "AaveArb: gas_price=%.4f gwei (base=%.4f + tip=%.1f) lucro≈$%.2f",
    ...
    def _reserves_list(self) -> list[str]:
        if not self._reserves:
            try:
                ...
                logger.info("AaveArb: Reserves Aave Arbitrum: %d activos", len(self._reserves))
            except Exception as exc:
                logger.warning("AaveArb: getReservesList falhou: %s", exc)
    ...
    def _scan_borrowers(self) -> None:
        ...
        try:
            ...
            logger.warning("AaveArb: não consegui ler bloco actual: %s", exc)
            return
        ...
        while from_block < latest:
            ...
            try:
                ...
                logger.debug("AaveArb: Borrow events [%d..%d]: %s", from_block, to_block, exc)
                break
        ...
        if added:
            logger.info("AaveArb: Scan Borrow: +%d mutuários (total=%d, bloco=%d)",
    ...
    def _check_health(self, address: str) -> tuple[float, float, float] | None:
        ...
        try:
            ...
            logger.debug("AaveArb: getUserAccountData(%s…): %s", address[:10], exc)
            return None

    def _estimate(self, borrower: str, hf: float,
                  col_usd: float, debt_usd: float) -> LiqOpportunityArb:
        reserves = self._reserves_list()
        debt_asset       = reserves[0] if reserves else WETH_ARB
        collateral_asset = reserves[1] if len(reserves) > 1 else reserves[0] if reserves else WETH_ARB
    ...
    def _record(self, opp: LiqOpportunityArb, executed: bool,
        ...
        logger.debug("AaveArb: BD %s id=%d %s HF=%.4f",
                     action, rec_id, opp.borrower[:10], opp.health_factor)

    def _execute_live(self, opp: LiqOpportunityArb, nonce: int) -> str | None:
        """Executa flash loan liquidation via contrato deployado."""
        if self.flash is None:
            logger.error(
                "AaveArb: flash_loan_contract não configurado — não é possível executar live"
            )
            return None
        try:
            ...
            try:
                ...
                logger.warning(
                    "AaveArb: simulação falhou %s — cooldown 2min: %s",
            ...
            logger.info("AaveArb: LIQUIDAÇÃO TX: %s", tx_hash.hex())
            return tx_hash.hex()
        except Exception as exc:
            if "HEALTH_FACTOR_NOT_BELOW_THRESHOLD" in str(exc):
                self._cooldown[opp.borrower.lower()] = time.time() + 300
                logger.warning(
                    "AaveArb: %s HF acima do threshold — cooldown 5min",
                    opp.borrower[:10] + "…",
                )
            else:
                logger.error("AaveArb: falha ao executar liquidação: %s", exc)
            return None
    ...
    def tick(self) -> list[dict]:
        if not self._connected():
            logger.warning("AaveArb: sem ligação ao RPC Arbitrum — tick saltado")
            return []
        ...
        if not self.dry_run:
            ...
            if _bal_wei < Web3.to_wei(0.005, 'ether'):
                logger.error(
                    "AaveArb: saldo insuficiente (%.6f ETH < 0.005) — tick saltado",
                    _bal_wei / 1e18,
                )
                return []
        ...
        logger.info(
            "AaveArb Tick: %d elegíveis | %d liquidáveis (HF<1.0) | %d cooldown | %d blacklist",
        ...
        for borrower, hf, col_usd, debt_usd in eligible:
            ...
            if _b_low in self._blacklist:
                ...
                if abs(hf - _bl_hf) / max(_bl_hf, 0.001) < 0.05:
                    logger.debug("AaveArb: %s blacklisted (HF=%.4f base=%.4f) — saltado",
                                 borrower[:10] + "…", hf, _bl_hf)
                    continue
                del self._blacklist[_b_low]
                self._fail_counts.pop(_b_low, None)
                logger.info("AaveArb: %s saiu da blacklist (HF %.4f→%.4f)",
            ...
            logger.info(
                "AaveArb LIQUIDAÇÃO %s HF=%.4f debt=$%.2f(%s) col=$%.2f(%s) lucro≈$%.4f dry=%s",
            ...
            if not self.dry_run and hf < _HF_LIQUIDATABLE and opp.net_profit_usd >= self.min_profit:
                ...
                if _until > _now:
                    logger.debug("AaveArb: %s em cooldown (%.0fs restantes) — saltado",
                    ...
                    if tx_hash is None:
                        ...
                        if _cnt >= _BLACKLIST_FAILS:
                            ...
                            logger.warning(
                                "AaveArb: %s adicionado à blacklist (%d falhas consecutivas)",
                                borrower[:10] + "…", _cnt,
                            )
                    else:
                        self._fail_counts.pop(_b_low, None)
                ...
                if executed:
                    ...
                    self.notifier.notify(
                        "trade_executed",
                        f"🔵 LIQUIDAÇÃO Arbitrum executada {borrower[:10]}… "
        ...
        if results:
            logger.info("AaveArb: %d oportunidades (%d checadas)", len(results), checked)
        return results
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("AAVE V3 ARBITRUM LIQUIDATOR — Smoke Test")
    print("=" * 60)

    bot = AaveLiquidatorArbBot()

    connected = bot._connected()
    print(f"\n[1] RPC Arbitrum conectado: {connected}")
    if not connected:
        print("    AVISO: sem conectividade — verificar ALCHEMY_ARB_URL no .env")
        sys.exit(0)
    ...
    print(f"[3] Reserves Aave V3 Arbitrum: {len(reserves)} activos")
    for r in reserves[:5]:
        print(f"    {r}")
    if len(reserves) > 5:
